...
float calculate_segment_progress(const PoseData* current_pose,
    ...
    for (int i = 0; i < main_joint_count; i++) {
        ...
        // 신뢰도 확인
        if (current_pose->landmarks[joint].inFrameLikelihood < MIN_CONFIDENCE_THRESHOLD ||
            start_pose->landmarks[joint].inFrameLikelihood < MIN_CONFIDENCE_THRESHOLD ||
            end_pose->landmarks[joint].inFrameLikelihood < MIN_CONFIDENCE_THRESHOLD) {
            continue;
        }
        
        // 시작점에서 현재점까지의 거리
        float start_to_current = distance_3d(
            &start_pose->landmarks[joint].position,
            &current_pose->landmarks[joint].position
        );
        
        // 시작점에서 끝점까지의 거리
        float start_to_end = distance_3d(
            &start_pose->landmarks[joint].position,
...
float segment_calculate_similarity(const PoseData* current_pose, const PoseData* target_pose) {
    ...
    for (int i = 0; i < main_joint_count; i++) {
        ...
        // 신뢰도 확인
        if (current_pose->landmarks[joint].inFrameLikelihood < MIN_CONFIDENCE_THRESHOLD ||
            target_pose->landmarks[joint].inFrameLikelihood < MIN_CONFIDENCE_THRESHOLD) {
            continue;
        }
...
void calculate_correction_vectors(const PoseData* current_pose,
    ...
    for (int i = 0; i < POSE_LANDMARK_COUNT; i++) {
        // 신뢰도 확인
        if (current_pose->landmarks[i].inFrameLikelihood < MIN_CONFIDENCE_THRESHOLD ||
            target_pose->landmarks[i].inFrameLikelihood < MIN_CONFIDENCE_THRESHOLD) {
            corrections[i] = (Point3D){0.0f, 0.0f, 0.0f};
            continue;
        }
        
        // 교정 벡터 계산 (목표 - 현재)
        corrections[i].x = target_pose->landmarks[i].position.x - current_pose->landmarks[i].position.x;
        corrections[i].y = target_pose->landmarks[i].position.y - current_pose->landmarks[i].position.y;
        corrections[i].z = target_pose->landmarks[i].position.z - current_pose->landmarks[i].position.z;
