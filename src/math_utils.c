...
Point2D calculate_center_point(const PoseData *pose) {
  ...
  for (int i = 0; i < JOINT_COUNT; i++) {
    if (pose->landmarks[i].inFrameLikelihood >
        0.5f) { // 신뢰도가 0.5 이상인 관절만 사용
...
Point2D calculate_center_point_selected(const PoseData *pose,
  ...
  for (int i = 0; i < joint_count; i++) {
    ...
    if (joint >= 0 && joint < JOINT_COUNT &&
        pose->landmarks[joint].inFrameLikelihood > 0.5f) {
...
void interpolate_pose(const PoseData *start, const PoseData *end, float t,
  ...
  for (int i = 0; i < JOINT_COUNT; i++) {
    result->landmarks[i].position.x =
        ...
        t * (end->landmarks[i].position.x - start->landmarks[i].position.x);
    result->landmarks[i].position.y =
        start->landmarks[i].position.y +
        t * (end->landmarks[i].position.y - start->landmarks[i].position.y);

    // 신뢰도는 두 포즈의 평균으로 계산
    result->landmarks[i].inFrameLikelihood =
        (start->landmarks[i].inFrameLikelihood +
         end->landmarks[i].inFrameLikelihood) *
        0.5f;
  }

  result->timestamp =
      start->timestamp + (uint64_t)(t * (end->timestamp - start->timestamp));
}
...
void scale_pose(const PoseData *pose, float scale_factor, PoseData *result) {
  ...
  for (int i = 0; i < JOINT_COUNT; i++) {
    result->landmarks[i].position.x =
        pose->landmarks[i].position.x * scale_factor;
    result->landmarks[i].position.y =
        pose->landmarks[i].position.y * scale_factor;
    result->landmarks[i].inFrameLikelihood =
...
void transform_pose(const PoseData *pose, float scale_factor,
  ...
  for (int i = 0; i < JOINT_COUNT; i++) {
    result->landmarks[i].position.x =
        pose->landmarks[i].position.x * scale_factor + offset->x;
    result->landmarks[i].position.y =
        pose->landmarks[i].position.y * scale_factor + offset->y;
    result->landmarks[i].inFrameLikelihood =
        pose->landmarks[i].inFrameLikelihood;
  }
  result->timestamp = pose->timestamp;
